...
class CI_Xmlrpcs extends CI_Xmlrpc
{
	...
	{
		// Periksa apakah $call adalah struct
		if ($call->kindOf() != 'struct') {
			return $this->multicall_error('notstruct');
		}
	
		// Periksa apakah ada kunci 'methodName' dalam 'struct'
		if (!isset($call->me['struct']['methodName'])) {
			return $this->multicall_error('nomethod');
		}
	
		$methName = $call->me['struct']['methodName'];
	
		// Ambil tipe dan nilai scalar dari 'methodName'
		$methNameArray = $methName->me;
		reset($methNameArray);
		$firstKey = key($methNameArray); // Ambil kunci pertama
		$scalar_type = $firstKey == $this->xmlrpcI4 ? $this->xmlrpcInt : $firstKey;
		$scalar_value = $methNameArray[$firstKey];
	
		// Periksa tipe dan nilai scalar
		...
		// Ambil 'params' dari 'struct'
		$params = isset($call->me['struct']['params']) ? $call->me['struct']['params'] : null;
	
		// Periksa apakah 'params' adalah array
		if (!is_array($params)) {
		...
		// Eksekusi pesan dan dapatkan hasil
