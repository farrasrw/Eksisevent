...
class ImageLoader
{
	...
	function FLoadImage($param = array(), $strType= 1, $imgPath='', $imgSubPathField='', $strImgField='', $defaultImageSize="600x300", $debug="fimageberita"){
		...
		if($strType!=0 && !empty($param)){
			...
			if($bolValid){
				...
				$strRootPath = $_SERVER{'DOCUMENT_ROOT'}.'/kusbiyanto';
	...
	function imageManipulation($sourceOriginalFile="", $desPath, $arrConfig ){
		...
		$fileName = end($arrDesPath);
		if (empty($fileName)) {
    	$fileName = $arrDesPath[count($arrDesPath) - 2];
		}
    ...
    function fimageclient($param,$strType,$strImgField='member_images'){
		
		$param = json_decode(json_encode($param),true);
		
		$path = './media/images/client/';
		
		//Image Galery 
		if($strImgField=='image_name'){
			$param['memberid'] = $param['memberid'].'/galery';
		}
        
        //echo '<pre>';print_r($param);echo '</pre>';
	
		$return = $this->FLoadImage($param ,$strType, $path, 'memberid' , $strImgField ,'640x320' );
		return $return;
