...
class Project extends MY_ThemeController {
    ...
    public function viewProject(){
        ...
        $projectId = $this->ffunction->decode( urlencode($this->input->get('project')) );
		...
		if(count($objproject)==1){
			...
			$arrData['hdnkey']= $objproject[0]->project_id; 
			$arrData['project'] = $objproject[0];
			//$arrData['taglist'] = $this->underscore->map($arrTag, function($num) { return $num['tag_name']; });
            
            //echo '<pre>';print_r($arrData['project']->project_name);echo '</pre>';
	...
	public function listprojectdata(){
		...
		foreach($arrData as $objproject){
            ...
            array_push($arrValue, 
					   "<center>
                       <a href=\"".base_url()."admin/project/viewProject.html?project=".$this->ffunction->encode($objproject->project_id)."\" title=\"Edit\"><img src=\"".base_url()."style/admin/images/edit.png\" /></a> &nbsp;
                       <a href=\"".base_url()."admin/project/deleteproject.html?project=".$this->ffunction->encode($objproject->project_id)."\" title=\"Delete\"><img src=\"".base_url()."style/admin/images/delete.png\" onclick=\"return confirm('Anda ingin menghapus data tersebut?')\" /></a>
                       </center>");
